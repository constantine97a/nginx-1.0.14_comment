...
/**
 * mytest_upstream_process_header方法可以解析HTTP响应头部，而这个例子只是简单地把上游服务器发送的
 * HTTP头部添加到了请求r->upstream->headers_in.headers链表中。
 * 如果有需要特殊处理的HTTP头部，那么也应该在mytest_upstream_process_header方法中进行
 */
static ngx_int_t
mytest_upstream_process_header(ngx_http_request_t *r)
{
